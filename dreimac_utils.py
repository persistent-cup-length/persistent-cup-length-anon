"""
Some functions from dreimac library (https://dreimac.scikit-tda.org) with minor modifications
as well as a few of our own helper functions
"""
import time
import numpy as np
...
@jit(nopython=True)
def compute_birth_time(new_a: List, distance_matrix: np.array) -> float:
    """
    compute the birth time of simplex new_a
    """
    # vertex
    if len(new_a) == 1:
        return 0
    # edge
        ...
        return distance_matrix[new_a[0], new_a[1]]
    # triangle
    elif len(new_a) == 3:
        # compute all possible pairs (edges)
        edges = create_pairs(new_a)
...
class CohomologyUtils:
    ...
    def make_delta1(dist_mat: np.ndarray, threshold: float, lookup_table: np.ndarray):
        n_points = dist_mat.shape[0]
        n_edges = (n_points * (n_points - 1)) // 2
        n_faces = number_of_simplices_of_dimension(2, n_points, lookup_table) # n_points choose 3
        ...
        def _delta1_get_row_columns_values(
            ...
            birth_times_rows = np.zeros(n_faces)
            birth_times_columns = np.zeros(n_edges)
            n_entries = 0
            for i in range(n_points):
                for j in range(i + 1, n_points):
                    if dist_mat[i, j] < threshold:
                        for k in range(j + 1, n_points):
                            if (
                                ...
                                # add birth times for each edge and triangle
                                # here, edges and triangles have vertices in increasing order, i.e.
                                # a triangle would be [v_0, v_1, v_2] where v_0 < v_1 < v_2
                                birth_times_rows[row_index] = compute_birth_time([i,j,k], dist_mat)
                                birth_times_columns[column_index_ij] = compute_birth_time([i, j], dist_mat)
                                birth_times_columns[column_index_jk] = compute_birth_time([j, k], dist_mat)
                                birth_times_columns[column_index_ik] = compute_birth_time([i, k], dist_mat)

            return n_entries, birth_times_rows, birth_times_columns

        n_entries, birth_times_rows_arr,  birth_times_columns_arr = _delta1_get_row_columns_values(
            dist_mat, threshold, lookup_table, n_points, rows, columns, values
        )
        ...
        ), birth_times_rows_arr, birth_times_columns_arr
