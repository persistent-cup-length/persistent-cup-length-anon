...
def get_spikes(rat_name, mod_name, day_name = '', sess_name = '', bType = None, bSmooth = True,
    ...
    if str(rat_name).upper() == 'R':
        f = np.load(folder + 'rat_r_' + day_name + '_grid_modules_1_2_3.npz', allow_pickle = True)
    elif str(rat_name).upper() == 'Q':
        f = np.load(folder + 'rat_q_grid_modules_1_2.npz', allow_pickle = True)
    elif str(rat_name).upper() == 'S':
        print('Folder: ', folder)
        #f = np.load('/Toroidal_topology_grid_cell_data/rat_s_grid_modules_1.npz', allow_pickle = True)
    ...
    if bType:
        f = np.load(folder + 'is_conjunctive_all.npz', allow_pickle = True)
        if len(day_name)>0:
            is_conj = f['is_conj_' + str(rat_name).upper() + mod_name + '_' + day_name]
        else:
            is_conj = f['is_conj_' + str(rat_name).upper() + mod_name + day_name]
        f.close()
        if bType == 'conj':
    ...
    if sess_name.find('_')>-1:
        ...
        ind = f[str(rat_name).upper() + mod_name + '_ind']
        f.close()
    ...
    if str(sess_name[:3]).upper() in ('SWS', 'REM'):
        times = times_all['rat_' + str(rat_name).lower() + '_sleep' + day_name]
    elif str(sess_name[:3]).upper() == 'WW2':
...
def load_pos(rat_name, sess_name, day_name = '', bSpeed = False, folder = '/grid_cell_analysis/Toroidal_topology_grid_cell_data/'):
    if str(rat_name).upper() == 'R':
        f = np.load(folder + 'rat_r_' + day_name + '_grid_modules_1_2_3.npz', allow_pickle = True)
    elif str(rat_name).upper() == 'Q':
        f = np.load(folder + 'rat_q_grid_modules_1_2.npz', allow_pickle = True)
    ...
    if str(sess_name).upper() in ('SWS', 'REM'):
        times = times_all['rat_' + str(rat_name).lower() + '_sleep' + day_name]
    else:
        times = times_all['rat_' + str(rat_name).lower() + '_' + sess_name + day_name]
    ...
    if (str(rat_name).upper() == 'R') & (day_name == 'day1'):
        if sess_name =='OF':
            valid_times = np.concatenate((np.arange(0, (14778-7457)*res/dt),
                                          np.arange((14890-7457)*res/dt, (16045-7457)*res/dt))).astype(int)
        elif sess_name == 'WW':
    ...
    if bSpeed:
        ...
        speed = np.concatenate(([speed[0]],speed))
        if (str(rat_name).upper() =='S') & (sess_name == 'WW'):
